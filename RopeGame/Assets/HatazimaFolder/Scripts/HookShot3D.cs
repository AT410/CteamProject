...
public class HookShot3D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0)) ObjectDesignation3D();
    ...
    void ObjectDesignation()
    {
        ...
        Ray2D ray = new Ray2D(transform.position, transform.TransformDirection(Vector3.up * 25f));
        RaycastHit2D hit = Physics2D.Raycast(ray.origin, ray.direction, 10f);
        Debug.DrawRay(ray.origin, ray.direction * 25, Color.red, 3);
